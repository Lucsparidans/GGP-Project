...
public class TunerAdaptivePlayoutAfterSimulation  extends AfterSimulationStrategy{

	private PpaWeights ppaWeights;


	// This version of adaptive after play-out has to have equal alpha and alpha loss since we want updated weights only for the winner
...
//	private double alphaLoss;
	...
	public TunerAdaptivePlayoutAfterSimulation(GameDependentParameters gameDependentParameters, Random random,
	...
	public void afterSimulationActions(SimulationResult[] simulationResult) {
		...
		for(int resultIndex = 0; resultIndex < simulationResult.length; resultIndex++){
			...
			double[] discountedAlphaLoss = new double[numRoles];
			for (int i = 0; i < numRoles; i++) {
				discountedAlpha[i] *= this.alpha.getValuePerRole(i);
				discountedAlphaLoss[i] *= this.alpha.getValuePerRole(i);
			}
			...
			for(int i = allMovesInAllStates.size()-1; i >= 0; i--){
				...
				for(int roleIndex = 0; roleIndex < updateFactors.length; roleIndex++){
					...
					if(updateFactors[roleIndex] != 0){
		    			...
		    			this.ppaWeights.adaptPolicyForRole(roleIndex, legalMovesForRole,
		    					roleMove, updateFactors[roleIndex], this.invert, discountedAlpha[roleIndex],
		    					discountedAlphaLoss[roleIndex], this.gameDependentParameters.getTotIterations(),
				...
				for (int ind = 0; ind < numRoles; ind++) {
					...
					discountedAlphaLoss[ind] *= this.alpha.getValuePerRole(ind);
				}
