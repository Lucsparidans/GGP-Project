...
public class TunerNppaAfterSimulation extends AfterSimulationStrategy {
...
//	private double alpha;
	private TunableParameter alpha;
...
//	private int maxNGramLength;
	private TunableParameter maxNGramLength;

	public TunerNppaAfterSimulation(GameDependentParameters gameDependentParameters, Random random,
		...
		this.alpha = this.createTunableParameter("AfterSimulationStrategy" + id, "Alpha", gamerSettings, sharedReferencesCollector);
		...
		this.maxNGramLength = this.createTunableParameter("AfterSimulationStrategy" + id, "MaxNGramLength", gamerSettings, sharedReferencesCollector);
	...
	public void clearComponent() {
		...
		this.alpha.clearParameter();
		this.maxNGramLength.clearParameter();
	}
	...
	public void setUpComponent() {
		...
		this.alpha.setUpParameter(this.gameDependentParameters.getNumRoles());
		this.maxNGramLength.setUpParameter(this.gameDependentParameters.getNumRoles());
	}
...
*/
	...
	private void updateAllRolesForPlayout(List<List<Move>> allJointMoves, List<List<List<Move>>> allMovesInAllStates, double[] rewards){
		...
		int numRoles = this.gameDependentParameters.getNumRoles();
		double[] discountedAlpha = new double[numRoles];
		for (int i = 0; i < numRoles; i++) {
			discountedAlpha[i] *= this.alpha.getValuePerRole(i);
		}
		...
		for(int jmIndex = allJointMoves.size()-1; jmIndex >= 0; jmIndex--){
			...
			for(int roleIndex = 0; roleIndex < this.gameDependentParameters.getNumRoles(); roleIndex++){
				...
				this.updateNGramsForRoleInState(roleIndex, jmIndex, allJointMoves, allMovesInAllStates, rewards[roleIndex], discountedAlpha[roleIndex]);

			}

			for (int ind = 0; ind < numRoles; ind++) {
				discountedAlpha[ind] *= this.alpha.getValuePerRole(ind);
			}
	...
	private void updateWinningRoleForPlayout(List<List<Move>> allJointMoves, List<List<List<Move>>> allMovesInAllStates, int winnerIndex){
		...
		double discountedAlpha = this.alpha.getValuePerRole(winnerIndex);
	...
	private void updateNGramsForRoleInState(int roleIndex, int jmIndex, List<List<Move>> allJointMoves,
		...
		while(currentNGramLength <= this.maxNGramLength.getValuePerRole(currentRoleIndex) && currentJmIndex >= 0){
